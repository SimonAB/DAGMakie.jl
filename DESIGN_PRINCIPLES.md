...
- **APIs speak Pearl/SciML for identification and estimation** (`do(·)`, backdoor adjustment, LMTP, EIF). Do not rename those exports. **Temporal declarations** use orthogonal fields on existing types: `temporal_support`, `value_representation`, `referent_id` / `ReferentSpec`, `identity_criterion`, `graph_kind`, optional `ontological_character`, `relation_kind`, and policy `information_set` (`ℋ_t`). Prefer `identity_criterion` and optional ontology on **`ReferentSpec`**, with variables pointing via `referent_id`. Node multiplicity follows support and graph kind only — never occasion/enduring, never glyphs. Deprecated `temporal_mode = :occasion | :enduring` is a constructor alias only and must not assert ontology; Documenter owns migration detail and must not invent defaults when the old flag underdetermines new semantics. Do **not** add a `node_expansion` shortcut. Do **not** overload `GlobalSupport()` as both unspecified support and time invariance — keep those as separate declarations. Glyphs follow `value_representation` under an explicit visual convention (e.g. rounded for interval summary); never endurance or cardinality. Whitehead glossary terms stay in the **book**. Documenter defines each computational term in one sentence, then uses the ordinary causal gloss. These names attach to existing types (`TemporalNodeSpec`, `LaggedEdge`, `Policy`); they do not justify a parallel semantic pipeline.
